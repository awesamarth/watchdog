...
- App Server `subAgentActivity` items carry the live mapping between a child thread ID and Codex's canonical agent path (for example `/root/runtime`), but Codex emits them from both parent and child viewpoints. Apply a path only after `thread/read`/`thread.started` topology proves the enclosing thread is the item's direct parent; otherwise a child-side `/root` back-reference can incorrectly re-parent the root.
