  ...
  const server = new McpServer(
    { name: "watchdog", version: "0.0.1" },
    {
      instructions: [
        ...
        "When a spawned child performs a node, pass the actual child thread ID returned by the spawn as ownerAgent; omitting ownerAgent truthfully attributes the attempt to the root.",
  ...
  server.registerTool(
    "watchdog_execution",
    {
      ...
      description: [
        ...
        "For spawned-child work, set ownerAgent to the actual thread ID returned by the spawn so Watchdog can correlate the node attempt.",
